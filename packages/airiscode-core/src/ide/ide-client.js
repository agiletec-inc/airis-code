...
export class IdeClient {
    ...
    async connect() {
        if (!this.currentIde) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run AIRIS Code in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
        ...
        const workspacePath = this.connectionConfig?.workspacePath ??
            process.env['AIRISCODE_IDE_WORKSPACE_PATH'];
    ...
    static validateWorkspacePath(ideWorkspacePath, cwd) {
        ...
        if (!isWithinWorkspace) {
            return {
                isValid: false,
                error: `Directory mismatch. AIRIS Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(', ')}`,
            };
        }
        return { isValid: true };
    }
    getPortFromEnv() {
        const port = process.env['AIRISCODE_IDE_SERVER_PORT'];
        if (!port) {
            return undefined;
        }
        return port;
    ...
    getStdioConfigFromEnv() {
        const command = process.env['AIRISCODE_IDE_SERVER_STDIO_COMMAND'];
        if (!command) {
            return undefined;
        }
        const argsStr = process.env['AIRISCODE_IDE_SERVER_STDIO_ARGS'];
        let args = [];
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('AIRISCODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
                debugLogger.error('Failed to parse AIRISCODE_IDE_SERVER_STDIO_ARGS:', e);
            }
        }
        return { command, args };
    }
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `airiscode-ide-server-${this.ideProcessInfo.pid}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
                // For older/newer extension versions, the file name matches the pattern
                // /^airiscode-ide-server-${pid}-\d+\.json$/. If multiple IDE
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `airiscode-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
