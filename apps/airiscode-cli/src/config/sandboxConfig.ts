...
function getSandboxCommand(sandbox?: boolean | string): SandboxConfig["command"] | "" {
  ...
  const environmentConfiguredSandbox = process.env["AIRISCODE_SANDBOX"]?.toLowerCase().trim() ?? "";
  ...
  if (typeof sandbox === "string" && sandbox) {
    ...
    throw new FatalSandboxError(`Missing sandbox command '${sandbox}' (from AIRISCODE_SANDBOX)`);
  }
  ...
  if (sandbox === true) {
    throw new FatalSandboxError(
      "AIRISCODE_SANDBOX is true but failed to determine command for sandbox; " +
        "install docker or podman or specify command in AIRISCODE_SANDBOX",
    );
  }
...
export async function loadSandboxConfig(
  ...
  const image =
    argv.sandboxImage ??
    process.env["AIRISCODE_SANDBOX_IMAGE"] ??
