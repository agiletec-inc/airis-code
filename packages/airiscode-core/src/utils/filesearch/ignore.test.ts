...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .airiscodeigenore', async () => {
    tmpDir = await createTmpDir({
      '.airiscodeigenore': '*.log',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: false,
  ...
  it('should combine rules from .gitignore and .airiscodeigenore', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': '*.log',
      '.airiscodeigenore': '*.txt',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: true,
