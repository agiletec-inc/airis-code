    ...
    if (requestedModel && allowedModels.has(requestedModel)) {
        ...
        if (requestedModel) {
            ...
            const extraMessage = isVisionModel
                ...
                : '';
            warnings.push(`Warning: Unsupported Qwen OAuth model '${requestedModel}', falling back to '${DEFAULT_AIRISCODE_MODEL}'.${extraMessage}`);
        }
        resolvedModel = DEFAULT_AIRISCODE_MODEL;
        modelSource = defaultSource(`fallback to '${DEFAULT_AIRISCODE_MODEL}'`);
    }
