  ...
  return [
    {
      ...
      description: t("Checked in at .airiscode/settings.json"),
      value: SettingScope.Workspace,
      key: "project",
    },
    {
      label: t("User settings"),
      description: t("Saved in at ~/.airiscode/settings.json"),
  ...
  return [
    {
      id: "allow",
      label: t("Allow"),
      description: t("AIRIS Code won't ask before using allowed tools."),
    },
    {
      id: "ask",
      label: t("Ask"),
      description: t("AIRIS Code will ask before using these tools."),
    },
    {
      id: "deny",
      label: t("Deny"),
      description: t("AIRIS Code is not allowed to use denied tools."),
    },
    {
      id: "workspace",
      label: t("Workspace"),
...
export function PermissionsDialog({ onExit }: PermissionsDialogProps): React.JSX.Element {
  ...
  const handleAddDirSubmit = useCallback(() => {
    ...
    const expanded = trimmed.startsWith("~") ? trimmed.replace(/^~/, os.homedir()) : trimmed;
    const absoluteExpanded = nodePath.isAbsolute(expanded) ? expanded : nodePath.resolve(expanded);
    ...
    const currentDirs = (settings.merged as Record<string, unknown>)["context"] as
      | Record<string, string[]>
      | undefined;
    const existingDirs = currentDirs?.["includeDirectories"] ?? [];
    if (!existingDirs.includes(resolved)) {
      settings.setValue(SettingScope.Workspace, key, [...existingDirs, resolved]);
    }
  ...
  const handleRemoveDirConfirm = useCallback(() => {
    ...
    for (const scope of [SettingScope.User, SettingScope.Workspace]) {
      ...
      const contextSection = (scopeSettings as Record<string, unknown>)["context"] as
        | Record<string, string[]>
        | undefined;
  ...
  const filteredRules = useMemo(() => {
    ...
    return currentTabRules.filter(
      (r) => r.rule.raw.toLowerCase().includes(q) || r.rule.toolName.toLowerCase().includes(q),
    );
  ...
  const handleScopeSelect = useCallback(
    (scope: SettingScope) => {
      ...
      const perms = (settings.merged as Record<string, unknown>)["permissions"] as
        | Record<string, string[]>
        | undefined;
  ...
  const handleDeleteConfirm = useCallback(() => {
    ...
    for (const scope of [SettingScope.User, SettingScope.Workspace]) {
      const scopeSettings = settings.forScope(scope).settings;
      const perms = (scopeSettings as Record<string, unknown>)["permissions"] as
        | Record<string, string[]>
        | undefined;
      const scopeRules = perms?.[ruleType];
      if (scopeRules?.includes(deleteTarget.rule.raw)) {
        const updated = scopeRules.filter((r: string) => r !== deleteTarget.rule.raw);
    ...
    return (
      <Box flexDirection="column">
        ...
        <Text color={theme.text.secondary} wrap="wrap">
          {t(
            "AIRIS Code will be able to read files in this directory and make edits when auto-accept edits is on.",
          )}
        </Text>
        <Box height={1} />
        ...
        >
          <TextInput
            ...
            onDown={dirCompletions.length > 0 ? handleDirCompletionDown : undefined}
  ...
  if (activeTab.id === "workspace" && view === "ws-remove-confirm" && removeDirTarget) {
    return (
      <Box flexDirection="column">
        <Box
          borderStyle="round"
        ...
        >
          ...
          <Box height={1} />
          <Text>{t("Are you sure you want to remove this directory from the workspace?")}</Text>
        </Box>
        <Box marginTop={1} marginLeft={1}>
          <Text color={theme.text.secondary}>{t("Enter to confirm · Esc to cancel")}</Text>
        </Box>
      </Box>
    );
  }
  ...
  if (activeTab.id === "workspace") {
    ...
    return (
      <Box flexDirection="column">
        ...
        <Text color={theme.text.secondary} wrap="wrap">
          {t(
            "AIRIS Code can read files in the workspace, and make edits when auto-accept edits is on.",
          )}
        </Text>
        <Box height={1} />
        ...
        {initialDirArray.map((dir, idx) => (
          <Box key={dir} marginLeft={2}>
            ...
            <Text>{dir}</Text>
            <Text color={theme.text.secondary}>
              {idx === 0 ? t("  (Original working directory)") : t("  (from settings)")}
            </Text>
          </Box>
        ))}
    ...
    return (
      <Box flexDirection="column">
        <Box
          ...
          padding={1}
        >
          <Text bold>{t("Add {{type}} permission rule", { type: activeTab.id })}</Text>
          <Box height={1} />
          <Text wrap="wrap">
            {t(
          ...
          <Text>
            {t("e.g.,")} <Text bold>WebFetch</Text> {t("or")} <Text bold>Bash(ls:*)</Text>
          </Text>
          <Box height={1} />
          <Box
            borderStyle="round"
        ...
        <Box marginTop={1} marginLeft={1}>
          <Text color={theme.text.secondary}>{t("Enter to submit · Esc to cancel")}</Text>
        </Box>
      </Box>
    );
  }
      ...
      <Box flexDirection="column">
        ...
        >
          ...
          <Box marginLeft={2} flexDirection="column">
            ...
            <Text color={theme.text.secondary}>{describeRule(pendingRuleText)}</Text>
          </Box>
          <Box height={1} />
    ...
    return (
      <Box flexDirection="column">
        <Box
          ...
          padding={1}
        >
          <Text bold>{t("Delete {{type}} rule?", { type: deleteTarget.type })}</Text>
          <Box height={1} />
          <Box marginLeft={2} flexDirection="column">
            <Text bold>{deleteTarget.rule.raw}</Text>
            <Text color={theme.text.secondary}>{describeRule(deleteTarget.rule.raw)}</Text>
            <Text color={theme.text.secondary}>{scopeLabel(deleteTarget.scope)}</Text>
          </Box>
          <Box height={1} />
  ...
  return (
    <Box flexDirection="column">
      ...
      >
        ...
        {searchQuery ? <Text>{searchQuery}</Text> : <Text color={Colors.Gray}>{t("Search…")}</Text>}
      </Box>
      <Box height={1} />
      {/* Rule list */}
      <RadioButtonSelect
...
function TabBar({ tabs, activeIndex }: { tabs: Tab[]; activeIndex: number }): React.JSX.Element {
  return (
    <Box marginBottom={1}>
      ...
      {tabs.map((tab, i) => (
        <Box key={tab.id} marginRight={2}>
          {i === activeIndex ? (
            <Text bold backgroundColor={theme.text.accent} color={theme.background.primary}>
              {` ${tab.label} `}
            </Text>
          ) : (
...
function FooterHint({ view }: { view: DialogView }): React.JSX.Element {
  ...
  return (
    <Box marginTop={1}>
      <Text color={theme.text.secondary}>
        {t("Press ↑↓ to navigate · Enter to select · Type to search · Esc to cancel")}
      </Text>
    </Box>
  );
}
