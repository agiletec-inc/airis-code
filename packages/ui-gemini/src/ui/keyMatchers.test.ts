...
import { Command, createKeyMatchers, keyMatchers } from "./keyMatchers.js";
...
describe("keyMatchers", () => {
  ...
  const originalMatchers: Record<Command, (key: Key) => boolean> = {
    ...
    [Command.COMPLETION_UP]: (key: Key) => key.name === "up" || (key.ctrl && key.name === "p"),
    [Command.COMPLETION_DOWN]: (key: Key) => key.name === "down" || (key.ctrl && key.name === "n"),
    [Command.ESCAPE]: (key: Key) => key.name === "escape",
    [Command.SUBMIT]: (key: Key) => key.name === "return" && !key.ctrl && !key.meta && !key.paste,
    ...
    [Command.SUBMIT_REVERSE_SEARCH]: (key: Key) => key.name === "return" && !key.ctrl,
    [Command.ACCEPT_SUGGESTION_REVERSE_SEARCH]: (key: Key) => key.name === "tab",
    [Command.TOGGLE_SHELL_INPUT_FOCUS]: (key: Key) => key.ctrl && key.name === "f",
  ...
  const testCases = [
    ...
    // Cursor movement
    {
      ...
      negative: [createKey("a"), createKey("a", { shift: true }), createKey("b", { ctrl: true })],
    },
    {
      command: Command.END,
      positive: [createKey("e", { ctrl: true })],
      negative: [createKey("e"), createKey("e", { shift: true }), createKey("a", { ctrl: true })],
    },
    ...
    },
    {
      ...
      positive: [createKey("backspace", { ctrl: true }), createKey("backspace", { meta: true })],
  ...
  describe("Data-driven key binding matches original logic", () => {
    ...
    it("should properly handle ACCEPT_SUGGESTION_REVERSE_SEARCH cases", () => {
      expect(
        keyMatchers[Command.ACCEPT_SUGGESTION_REVERSE_SEARCH](createKey("return", { ctrl: true })),
      ).toBe(false); // ctrl must be false
      expect(keyMatchers[Command.ACCEPT_SUGGESTION_REVERSE_SEARCH](createKey("tab"))).toBe(true);
      expect(
        keyMatchers[Command.ACCEPT_SUGGESTION_REVERSE_SEARCH](createKey("tab", { ctrl: true })),
