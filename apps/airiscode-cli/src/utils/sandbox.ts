...
export async function start_sandbox(
  ...
  const isIntegrationTest =
    process.env['AIRISCODE_INTEGRATION_TEST'] === 'true';
  let containerName;
  if (isIntegrationTest) {
    containerName = `airiscode-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  if (process.env['AIRISCODE_TEST_VAR']) {
    args.push(
      '--env',
      `AIRISCODE_TEST_VAR=${process.env['AIRISCODE_TEST_VAR']}`,
    );
  }
