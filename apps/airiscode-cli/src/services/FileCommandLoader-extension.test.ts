...
describe('FileCommandLoader - Extension Commands Support', () => {
  ...
  const projectCommandsDir = path.join(projectRoot, '.airiscode', 'commands');
  ...
  it('should load commands from multiple extensions in alphabetical order', async () => {
    const ext1Dir = path.join(projectRoot, '.airiscode', 'extensions', 'ext-b');
    const ext2Dir = path.join(projectRoot, '.airiscode', 'extensions', 'ext-a');
