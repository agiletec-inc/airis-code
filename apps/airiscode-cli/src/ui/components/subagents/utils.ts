...
export const shouldShowColor = (color?: string): boolean => color !== undefined && color !== "auto";
...
export function getTotalSteps(method: "airiscode" | "manual"): number {
  return method === "manual" ? 8 : TOTAL_WIZARD_STEPS;
}

export function getStepKind(method: "airiscode" | "manual", stepNumber: number): StepKind {
  if (method === "manual") {
    switch (stepNumber) {
      case 1:
        return "LOCATION";
