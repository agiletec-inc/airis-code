...
`packages/runtime` + `packages/cli`, add an Ollama backend, and migrate the Claude Code
...
- `packages/airiscode-runtime/` — `@airiscode/runtime`. Copied from Qwen `packages/runtime`
...
concepts (MCP gateway integration, policies) will be re-homed into `airiscode-runtime`;
...
Rule of thumb: new features go in `packages/airiscode-runtime` or `apps/airiscode-cli`.

## Development Commands

Monorepo tooling: **pnpm + Turbo**. All config (`package.json`, `pnpm-workspace.yaml`,
`tsconfig*.json`) is hand-managed — edit it directly. pnpm settings live in
`pnpm-workspace.yaml` (the SSoT on pnpm 11+); the workspace pins pnpm via the root
...
# Scoped builds / tests
pnpm --filter @airiscode/runtime build
...
Both `@airiscode/runtime` and `@airiscode/cli` use plain `tsc` for build (not tsup) — if a
...
Keep the diff surface minimal so future re-syncs with upstream stay tractable.
...
1. **Layer 1**: `packages/airiscode-runtime/src/mcp/` (inherited from Qwen) — the path
...
- `docs/MCP_INTEGRATION.md`, `docs/TOOL_EXECUTION_LOOP.md`,
  ...
  fork; cross-check with current `packages/airiscode-runtime` code).
