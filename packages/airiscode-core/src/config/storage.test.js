...
describe('Storage – getGlobalSettingsPath', () => {
    it('returns path to ~/.airiscode/settings.json', () => {
        const expected = path.join(os.homedir(), '.airiscode', 'settings.json');
...
describe('Storage – additional helpers', () => {
    ...
    it('getWorkspaceSettingsPath returns project/.airiscode/settings.json', () => {
        const expected = path.join(projectRoot, '.airiscode', 'settings.json');
        expect(storage.getWorkspaceSettingsPath()).toBe(expected);
    });
    it('getUserCommandsDir returns ~/.airiscode/commands', () => {
        const expected = path.join(os.homedir(), '.airiscode', 'commands');
        expect(Storage.getUserCommandsDir()).toBe(expected);
    });
    it('getProjectCommandsDir returns project/.airiscode/commands', () => {
        const expected = path.join(projectRoot, '.airiscode', 'commands');
        expect(storage.getProjectCommandsDir()).toBe(expected);
    });
    it('getMcpOAuthTokensPath returns ~/.airiscode/mcp-oauth-tokens.json', () => {
        const expected = path.join(os.homedir(), '.airiscode', 'mcp-oauth-tokens.json');
...
describe('Storage – getRuntimeBaseDir / setRuntimeBaseDir', () => {
    ...
    it('resolves relative paths in setRuntimeBaseDir using explicit cwd', () => {
        ...
        Storage.setRuntimeBaseDir('.airiscode', cwd);
        expect(Storage.getRuntimeBaseDir()).toBe(path.join(cwd, '.airiscode'));
    });
...
describe('Storage – config paths remain at ~/.qwen regardless of runtime dir', () => {
    ...
    it('getUserSkillsDirs still includes ~/.airiscode/skills', () => {
        const storage = new Storage('/tmp/project');
...
describe('Storage – runtime base dir async context isolation', () => {
    ...
    it('uses contextual runtime dir inside runWithRuntimeBaseDir', async () => {
        ...
        await Storage.runWithRuntimeBaseDir('.airiscode', cwd, async () => {
            expect(Storage.getRuntimeBaseDir()).toBe(path.join(cwd, '.airiscode'));
        });
    });
