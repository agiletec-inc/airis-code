...
const createMockConfig = (overrides = {}) => ({
  ...
  getTargetDir: vi.fn(() => '/projects/airiscode'),
  getMcpServers: vi.fn(() => ({})),
...
describe('<AppHeader />', () => {
  it('shows the working directory', () => {
    ...
    expect(lastFrame()).toContain('/projects/airiscode');
  });
  ...
  it('hides the header when screen reader is enabled', () => {
    ...
    expect(lastFrame()).not.toContain('/projects/airiscode');
    expect(lastFrame()).not.toContain('AIRIS Code');
  });

  it('shows the header with all info when banner is visible', () => {
    const { lastFrame } = renderWithProviders(createMockUIState());
    expect(lastFrame()).toContain('>_ AIRIS Code');
