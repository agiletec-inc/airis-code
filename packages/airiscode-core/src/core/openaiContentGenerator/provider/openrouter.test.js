...
describe('OpenRouterOpenAICompatibleProvider', () => {
    ...
    describe('buildHeaders', () => {
        it('should include base headers from parent class', () => {
            ...
            expect(headers['User-Agent']).toBe(`AirisCode/1.0.0 (${process.platform}; ${process.arch})`);
        });
        it('should add OpenRouter-specific headers', () => {
            const headers = provider.buildHeaders();
            expect(headers).toEqual({
                'User-Agent': `AirisCode/1.0.0 (${process.platform}; ${process.arch})`,
        ...
        it('should override parent headers if there are conflicts', () => {
            ...
            expect(headers).toEqual({
                'User-Agent': 'ParentAgent/1.0.0',
                'HTTP-Referer': 'https://github.com/QwenLM/airiscode.git', // OpenRouter-specific value should override
                'X-OpenRouter-Title': 'AIRIS Code',
            });
        ...
        it('should handle unknown CLI version from parent', () => {
            ...
            const headers = provider.buildHeaders();
            expect(headers['User-Agent']).toBe(`AirisCode/unknown (${process.platform}; ${process.arch})`);
            expect(headers['HTTP-Referer']).toBe('https://github.com/QwenLM/airiscode.git');
            expect(headers['X-OpenRouter-Title']).toBe('AIRIS Code');
        });
    });
    ...
    describe('integration with parent class', () => {
        ...
        it('should maintain parent functionality while adding OpenRouter specifics', () => {
            ...
            expect(headers['HTTP-Referer']).toBe('https://github.com/QwenLM/airiscode.git'); // OpenRouter-specific
            expect(headers['X-OpenRouter-Title']).toBe('AIRIS Code'); // OpenRouter-specific
        });
    });
});
