...
import { getSupportedLanguageIds, SUPPORTED_LANGUAGES } from "../../i18n/languages.js";
import {
  isAutoLanguage,
...
function getCurrentOutputLanguage(context?: CommandContext): {
  ...
  const settingValue =
    context?.services?.settings?.merged?.general?.outputLanguage || OUTPUT_LANGUAGE_AUTO;
    ...
    if (trimmedArgs) {
      ...
      if (subCommandName === "ui" || subCommandName === "output") {
        ...
        if (subCommand?.action) {
          return subCommand.action(context, subArgs) as Promise<SlashCommandActionReturn>;
        }
      }
