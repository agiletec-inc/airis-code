...
function buildArenaExecutionInput(
  ...
  if (!parsed.task) {
    return {
      ...
      content:
        ...
        '  /arena start --models airiscoder-plus,kimi-for-coding "fix the bug"',
    };
  }
  ...
  const models: ArenaModelConfig[] = parsed.models.map((parsedModel) => {
    const authType = (parsedModel.authType as AuthType | undefined) ?? defaultAuthType;
...
function executeArenaCommand(
  ...
  const addArenaMessage = (type: "info" | "warning" | "error" | "success", text: string) => {
  ...
  const buildAgentCardData = (result: ArenaAgentCompleteEvent["result"]): ArenaAgentCardData => ({
  ...
  subCommands: [
    ...
    },
    {
      ...
      action: async (
        context: CommandContext,
        args: string,
      ): Promise<void | MessageActionReturn | OpenDialogActionReturn | ConfirmActionReturn> => {
        const executionMode = context.executionMode ?? "interactive";
        if (executionMode !== "interactive") {
          return {
            type: "message",
