...
import { createTmpDir, cleanupTmpDir } from '@airiscode/test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .airiscodeigenore rules', async () => {
    tmpDir = await createTmpDir({
      '.airiscodeigenore': 'dist/',
      dist: ['ignored.js'],
      src: ['not-ignored.js'],
    });
    ...
    expect(results).toEqual(['src/', '.airiscodeigenore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .airiscodeigenore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.airiscodeigenore': 'build/',
