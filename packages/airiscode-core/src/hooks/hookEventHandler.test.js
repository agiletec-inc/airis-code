...
import { HookEventName, HookType, HooksConfigSource, SessionStartSource, SessionEndReason, PermissionMode, AgentType, PreCompactTrigger, NotificationType, } from './types.js';
...
describe('HookEventHandler', () => {
    ...
    describe('fireNotificationEvent', () => {
        ...
        it('should include all parameters in the hook input', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireNotificationEvent('AIRIS Code needs your permission to use Bash', NotificationType.PermissionPrompt, 'Permission needed');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.message).toBe('AIRIS Code needs your permission to use Bash');
        ...
        it('should pass notification_type as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.fireNotificationEvent('AIRIS Code is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input');
