...
import { buildHumanReadableRuleLabel, IdeClient, ToolConfirmationOutcome } from "@airiscode/runtime";
...
export const ToolConfirmationMessage: React.FC<ToolConfirmationMessageProps> = ({
  ...
  if (compactMode) {
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box>
          <RadioButtonSelect items={compactOptions} onSelect={handleSelect} isFocused={isFocused} />
        </Box>
      </Box>
    );
  }
  ...
  const options: Array<RadioSelectItem<ToolConfirmationOutcome>> =
    [] as RadioSelectItem<ToolConfirmationOutcome>[];
    ...
    bodyContent = (
      <Box flexDirection="column">
        <Box paddingX={1} marginLeft={1}>
          <MaxSizedBox maxHeight={bodyContentHeight} maxWidth={Math.max(contentWidth, 1)}>
            <Box>
