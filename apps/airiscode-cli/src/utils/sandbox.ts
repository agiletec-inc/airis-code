...
import { SETTINGS_DIRECTORY_NAME, USER_SETTINGS_DIR } from "../config/settings.js";
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = "airiscode-sandbox";
const SANDBOX_NETWORK_NAME = "airiscode-sandbox";
const SANDBOX_PROXY_NAME = "airiscode-sandbox-proxy";
...
export async function start_sandbox(
  ...
  if (config.command === "sandbox-exec") {
    ...
    // start and set up proxy if AIRISCODE_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env["AIRISCODE_SANDBOX_PROXY_COMMAND"];
  ...
  // determine full path for airiscode to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under airiscode repo
  //
  // note this can only be done with binary linked from airiscode repo
  if (process.env["BUILD_SANDBOX"]) {
    if (!gcPath.includes("airiscode/packages/")) {
      throw new FatalSandboxError(
        "Cannot build sandbox using installed AIRIS Code binary; " +
          "run `npm link ./packages/cli` under AirisCode-cli repo to switch to linked binary.",
      );
    } else {
      ...
      execSync(`cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`, {
        ...
        env: {
          ...process.env,
          AIRISCODE_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
        },
      });
    }
  }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? "Try running `npm run build:all` or `npm run build:sandbox` under the airiscode repo to build it locally, or check the image name and your network connection."
        : "Please check the image name, your network connection, or notify airiscode-dev@service.alibaba.com if the issue persists.";
    throw new FatalSandboxError(
  ...
  // AIRISCODE_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env["AIRISCODE_SANDBOX_PROXY_COMMAND"];
  ...
  const isIntegrationTest = process.env["AIRISCODE_INTEGRATION_TEST"] === "true";
  let containerName;
  if (isIntegrationTest) {
    containerName = `airiscode-integration-test-${randomBytes(4).toString("hex")}`;
  ...
  // copy AIRISCODE_TEST_VAR for integration tests
  if (process.env["AIRISCODE_TEST_VAR"]) {
    args.push("--env", `AIRISCODE_TEST_VAR=${process.env["AIRISCODE_TEST_VAR"]}`);
  }
  ...
  for (const envVar of [
    "AIRISCODE_IDE_SERVER_PORT",
    "AIRISCODE_IDE_WORKSPACE_PATH",
    "TERM_PROGRAM",
  ]) {
