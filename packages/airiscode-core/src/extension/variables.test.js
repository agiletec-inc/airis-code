...
describe('substituteHookVariables', () => {
    ...
    it('should handle multiple event types with hooks', () => {
        const basePath = '/home/user/.airiscode/extensions/my-extension';
        const hooks = {
            PreToolUse: [
                {
        ...
        expect(result).toBeDefined();
        expect(result['PreToolUse']).toHaveLength(1);
        expect(result['PreToolUse'][0].hooks[0].command).toBe('/home/user/.airiscode/extensions/my-extension/scripts/pre-start.sh');
        expect(result['UserPromptSubmit']).toHaveLength(1);
        expect(result['UserPromptSubmit'][0].hooks[0].command).toBe('/home/user/.airiscode/extensions/my-extension/setup/install.py');
    });
...
describe('performVariableReplacement', () => {
    ...
    it('should replace .claude with .qwen in markdown files', () => {
        ...
        expect(result).toContain('.airiscode/loop.local.md');
        expect(result).toContain('rm .airiscode/loop.local.md');
        expect(result).toContain('$HOME/.airiscode/cache');
        expect(result).toContain('./.airiscode/local');
    ...
    it('should replace .claude with .qwen in shell scripts', () => {
        ...
        expect(result).toContain('~/.airiscode/cache');
        expect(result).toContain('./.airiscode/local');
        expect(result).toContain('.airiscode/config');
        // Note: URLs are also being replaced in current implementation
        expect(result).toContain('https://example.com/.airiscode/page');
    });
