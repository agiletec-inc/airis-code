...
describe('modelConfigResolver', () => {
  describe('resolveModelConfig', () => {
    describe('OpenAI auth type', () => {
      ...
      it('reads AIRISCODE_MODEL as fallback for OPENAI_MODEL', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {},
          env: {
            AIRISCODE_MODEL: 'qwen-model',
            OPENAI_API_KEY: 'key',
          },
        });

        expect(result.config.model).toBe('qwen-model');
        expect(result.sources['model'].envKey).toBe('AIRISCODE_MODEL');
      });
    });
    ...
    describe('Qwen OAuth auth type', () => {
      it('uses default model for Qwen OAuth', () => {
        ...
        expect(result.config.model).toBe(DEFAULT_AIRISCODE_MODEL);
