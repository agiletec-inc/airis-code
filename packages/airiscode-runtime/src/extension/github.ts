...
export async function cloneFromGit(
  ...
  try {
    ...
    await git.clone(sourceUrl, "./", ["-c", `core.symlinks=${symlinkValue}`, "--depth", "1"]);
    ...
    if (entries.length === 2) {
      ...
      if (lonelyDir) {
        ...
        const hasMarketplaceConfig = fs.existsSync(
          path.join(destination, lonelyDir.name, ".claude-plugin/marketplace.json"),
        );
        const hasClaudePluginConfig = fs.existsSync(
          path.join(destination, lonelyDir.name, ".claude-plugin/plugin.json"),
        );
        if (hasQwenConfig || hasGeminiConfig || hasMarketplaceConfig || hasClaudePluginConfig) {
