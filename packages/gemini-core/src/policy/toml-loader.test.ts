...
async function runLoadPoliciesFromToml(
  ...
  const mockReaddir = vi.fn(
    async (path: string, _options?: { withFileTypes: boolean }): Promise<Dirent[]> => {
  ...
  const mockReadFile = vi.fn(async (path: string): Promise<string> => {
    if (nodePath.normalize(path) === nodePath.normalize(nodePath.join("/policies", fileName))) {
      return tomlContent;
    }
...
`);
    ...
    it("should handle a mix of valid and invalid policy files", async () => {
      ...
      const mockReadFile = vi.fn(async (path: string): Promise<string> => {
        if (
          nodePath.normalize(path) === nodePath.normalize(nodePath.join("/policies", "valid.toml"))
        ) {
          return `
[[rule]]
toolName = "glob"
