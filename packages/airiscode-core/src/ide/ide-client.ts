...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    const workspacePath =
      this.connectionConfig?.workspacePath ?? process.env["AIRISCODE_IDE_WORKSPACE_PATH"];
  ...
  private async getConnectionConfigFromFile(): Promise<IdeConnectionConfig | undefined> {
    const portFromEnv = this.getPortFromEnv();
    ...
    if (this.ideProcessInfo) {
      try {
        const portFile = path.join(
          os.tmpdir(),
          `airiscode-ide-server-${this.ideProcessInfo.pid}.json`,
        );
  ...
  protected async getAllConnectionConfigs(ideDir: string): Promise<IdeConnectionConfig[]> {
