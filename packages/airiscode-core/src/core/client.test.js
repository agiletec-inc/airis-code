...
describe('Gemini Client (client.ts)', () => {
    ...
    describe('thinking block idle cleanup and latch', () => {
        ...
        it('should not strip thoughts on active session (< 5min idle)', async () => {
            ...
            const gen = client.sendMessageStream([{ text: 'Hello' }], new AbortController().signal, 'prompt-1', { type: SendMessageType.UserQuery });
            for await (const _ of gen) {
                /* drain */
            }
            expect(mockChat.stripThoughtsFromHistoryKeepRecent).not.toHaveBeenCalled();
        });
        it('should latch and strip thoughts after > 5min idle', async () => {
            ...
            client['thinkingClearLatched'] = false;
            const gen = client.sendMessageStream([{ text: 'Hello' }], new AbortController().signal, 'prompt-2', { type: SendMessageType.UserQuery });
            for await (const _ of gen) {
                /* drain */
            }
            expect(client['thinkingClearLatched']).toBe(true);
            expect(mockChat.stripThoughtsFromHistoryKeepRecent).toHaveBeenCalledWith(1);
        });
        it('should keep stripping once latched even if idle < 5min', async () => {
            ...
            const gen = client.sendMessageStream([{ text: 'Hello' }], new AbortController().signal, 'prompt-3', { type: SendMessageType.UserQuery });
            for await (const _ of gen) {
                /* drain */
            }
            expect(client['thinkingClearLatched']).toBe(true);
