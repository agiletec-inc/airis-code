...
export class GeminiChat {
        ...
        return (async function* () {
            try {
                ...
                if (lastError === null &&
                    ...
                    debugLogger.info(`Output truncated at capped default. Escalating to ${ESCALATED_MAX_TOKENS} tokens.`);
    ...
    stripThoughtsFromHistoryKeepRecent(keepTurns) {
        ...
        const turnsToStrip = new Set(modelTurnIndices.slice(0, Math.max(0, modelTurnIndices.length - keepTurns)));
        if (turnsToStrip.size === 0)
            return;
        this.history = this.history
            .map((content, index) => {
            if (!turnsToStrip.has(index) || !content.parts)
                return content;
