...
import { HookEventName, HooksConfigSource } from "@airiscode/runtime";
import { t } from "../../../i18n/index.js";
import type { HookEventDisplayInfo, HookExitCode } from "./types.js";
...
export function getHookShortDescription(eventName: string): string {
  const descriptions: Record<string, string> = {
    ...
    [HookEventName.Stop]: t("Right before AIRIS Code concludes its response"),
 ...
 */
export function getTranslatedSourceDisplayMap(): Record<HooksConfigSource, string> {
  return {
 ...
 */
export const DISPLAY_HOOK_EVENTS: HookEventName[] = Object.values(HookEventName);

/**
 * Create empty hook event display info
 */
export function createEmptyHookEventInfo(eventName: HookEventName): HookEventDisplayInfo {
  return {
    event: eventName,
