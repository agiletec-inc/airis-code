...
export function HooksListStep({ hooks, selectedIndex }: HooksListStepProps): React.JSX.Element {
    ...
    <Box flexDirection="column" paddingX={1}>
      ...
      <Box marginBottom={1}>
        <Text color={theme.text.secondary}>
          {t(
            "This menu is read-only. To add or modify hooks, edit settings.json directly or ask AIRIS Code.",
          )}
        </Text>
      </Box>
          ...
          <Box key={hook.event}>
            ...
            <Box width={hookNameWidth}>
              <Text color={isSelected ? theme.text.accent : theme.text.primary} bold={isSelected}>
                ...
                {configCount > 0 && <Text color={theme.status.success}> ({configCount})</Text>}
              </Text>
            </Box>
      ...
      <Box marginTop={1}>
        <Text color={theme.text.secondary}>{t("Enter to select · Esc to cancel")}</Text>
      </Box>
    </Box>
  );
}
