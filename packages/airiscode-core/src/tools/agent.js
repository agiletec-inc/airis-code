...
class AgentToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput) {
        try {
            ...
            const subagent = await this.subagentManager.createAgentHeadless(subagentConfig, this.config, { eventEmitter: this.eventEmitter });
            ...
            if (hookSystem && !signal?.aborted) {
                ...
                while (continueExecution) {
                    ...
                    if (iterationCount >= maxIterations) {
                        debugLogger.warn(`[TaskTool] SubagentStop hook reached maximum iterations (${maxIterations}), forcing stop to prevent infinite loop`);
