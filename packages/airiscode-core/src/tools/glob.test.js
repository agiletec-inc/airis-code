...
describe('GlobTool', () => {
    ...
    describe('ignore file handling', () => {
        ...
        it('should respect .airiscodeigenore files by default', async () => {
            await fs.writeFile(path.join(tempRootDir, '.airiscodeigenore'), '*.airiscodeigenored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.airiscodeigenored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.notignored.txt'), 'not ignored content');
            // Recreate the tool to pick up the new .airiscodeigenore file
            globTool = new GlobTool(mockConfig);
            const params = { pattern: '*.txt' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
            expect(result.llmContent).toContain('Found 3 file(s)'); // fileA.txt, FileB.TXT, b.notignored.txt
            expect(result.llmContent).not.toContain('a.airiscodeigenored.txt');
        });
        ...
        it('should respect .airiscodeigenore when searching a subdirectory (path option)', async () => {
            await fs.writeFile(path.join(tempRootDir, '.airiscodeigenore'), '*.secret');
            await fs.writeFile(path.join(tempRootDir, 'sub', 'visible.txt'), 'ok');
            await fs.writeFile(path.join(tempRootDir, 'sub', 'hidden.secret'), 'should be ignored');
            // Recreate to pick up .airiscodeigenore
            const subDirTool = new GlobTool(mockConfig);
            const params = { pattern: '*', path: 'sub' };
            const invocation = subDirTool.build(params);
            const result = await invocation.execute(abortSignal);
