...
import { saveCacheSafeParams, getCacheSafeParams, clearCacheSafeParams, runForkedQuery, } from './forkedQuery.js';
...
describe('runForkedQuery', () => {
    ...
    it('preserves tools: [] even when jsonSchema is provided', async () => {
        ...
        const result = await runForkedQuery({}, 'suggest', {
            jsonSchema: schema,
        });
        const sendParams = capturedParams;
    ...
    it('throws when CacheSafeParams are not available', async () => {
        const mockConfig = {};
        await expect(runForkedQuery(mockConfig, 'test')).rejects.toThrow('CacheSafeParams not available');
    });
});
//# sourceMappingURL=forkedQuery.test.js.map