...
import { initialWizardState, wizardReducer } from "../reducers.js";
 ...
 */
export function AgentCreationWizard({ onClose, config }: AgentCreationWizardProps) {
  ...
  const renderStepFooter = useCallback(() => {
    const getNavigationInstructions = () => {
      ...
      const escAction =
        state.currentStep === WIZARD_STEPS.LOCATION_SELECTION ? t("cancel") : t("go back");
      ...
      case "MANUAL_NAME":
        return (
          <TextEntryStep
            ...
            validate={(text) => (text.trim().length === 0 ? t("Name cannot be empty.") : null)}
          />
        );
      case "MANUAL_PROMPT":
        return (
          <TextEntryStep
            ...
            validate={(text) =>
              text.trim().length === 0 ? t("System prompt cannot be empty.") : null
            }
          />
        );
      case "MANUAL_DESC":
        return (
          <TextEntryStep
            ...
            dispatch={dispatch}
            onNext={handleNext}
            description={t("Describe when and how this subagent should be used.")}
            placeholder={t("e.g., Reviews code for best practices and potential bugs.")}
            height={6}
            ...
            validate={(text) =>
              text.trim().length === 0 ? t("Description cannot be empty.") : null
            }
          />
        );
      case "TOOLS":
