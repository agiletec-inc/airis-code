...
import { AuthType, DEFAULT_AIRISCODE_MODEL } from '@airiscode/core';
...
const renderComponent = (
  ...
  const mockConfig = {
    ...
    getModel: vi.fn(() => DEFAULT_AIRISCODE_MODEL),
...
describe('<ModelDialog />', () => {
  ...
  it('passes all model options to DescriptiveRadioButtonSelect', () => {
    ...
    expect(props.items[0].value).toBe(
      `${AuthType.QWEN_OAUTH}::${DEFAULT_AIRISCODE_MODEL}`,
    );
    expect(props.showNumbers).toBe(true);
  });

  it('initializes with the model from ConfigContext', () => {
    const mockGetModel = vi.fn(() => DEFAULT_AIRISCODE_MODEL);
    renderComponent(
      {},
      {
    ...
    const expectedIndex = qwenModels.findIndex(
      (m) => m.id === DEFAULT_AIRISCODE_MODEL,
    );
  ...
  it('initializes with default coder model if getModel returns undefined', () => {
    ...
    // When getModel returns undefined, preferredModel falls back to DEFAULT_AIRISCODE_MODEL
  ...
  it('calls config.switchModel and onClose when DescriptiveRadioButtonSelect.onSelect is triggered', async () => {
    ...
    await childOnSelect(`${AuthType.QWEN_OAUTH}::${DEFAULT_AIRISCODE_MODEL}`);
  ...
  it('updates initialIndex when config context changes', () => {
    ...
    // DEFAULT_AIRISCODE_MODEL (coder-model) is at index 0
    expect(mockedSelect.mock.calls[0][0].initialIndex).toBe(0);

    mockGetModel.mockReturnValue(DEFAULT_AIRISCODE_MODEL);
    const newMockConfig = {
      getModel: mockGetModel,
      getAuthType: mockGetAuthType,
    ...
    // Calculate expected index for DEFAULT_AIRISCODE_MODEL dynamically
    const qwenModels = getFilteredQwenModels();
