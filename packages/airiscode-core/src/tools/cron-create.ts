...
class CronCreateInvocation extends BaseToolInvocation<CronCreateParams, ToolResult> {
  constructor(
    ...
    try {
      ...
      if (recurring) {
        llmContent =
          ...
          "Session-only (not written to disk, dies when AIRIS Code exits). " +
...
export class CronCreateTool extends BaseDeclarativeTool<CronCreateParams, ToolResult> {
  ...
  constructor(private config: Config) {
    super(
      ...
      "Schedule a prompt to be enqueued at a future time. Use for both recurring schedules and one-shot reminders.\n\n" +
        ...
        "## Session-only\n\n" +
        "Jobs live only in this AIRIS Code session — nothing is written to disk, and the job is gone when AIRIS Code exits.\n\n" +
        "## Runtime behavior\n\n" +
