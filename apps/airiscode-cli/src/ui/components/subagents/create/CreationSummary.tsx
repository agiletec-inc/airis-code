...
export function CreationSummary({
        ...
        if (!isAvailable) {
          ...
          if (existing) {
            ...
            if (conflictLevel === targetLevel) {
              ...
              allWarnings.push(
                t('Name "{{name}}" exists at user level - project level will take precedence', {
  ...
  const saveSubagent = useCallback(async (): Promise<SubagentManager> => {
    ...
    const subagentConfig: SubagentConfig = {
      ...
      tools: Array.isArray(state.selectedTools) ? state.selectedTools : undefined,
      color: state.color,
    };
  ...
  }, [saveSubagent, showSuccessAndClose, state.generatedName, state.location, launchEditor]);
    ...
    <Box flexDirection="column" gap={1}>
      <Box flexDirection="column">
        <Box>
          ...
          <Text color={getColorForDisplay(state.color)}>{state.generatedName}</Text>
        </Box>
        ...
        <Box>
          ...
          <Text>
            {state.location === "project"
              ? t("Project Level (.airiscode/agents/)")
              : t("User Level (~/.airiscode/agents/)")}
          </Text>
        </Box>
        ...
        <Box padding={1} paddingBottom={0}>
          <Text wrap="wrap">{truncateText(state.generatedDescription, 250)}</Text>
        </Box>
        ...
        <Box padding={1} paddingBottom={0}>
          <Text wrap="wrap">{truncateText(state.generatedSystemPrompt, 250)}</Text>
        </Box>
      </Box>
