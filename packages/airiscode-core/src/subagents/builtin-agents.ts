...
2. For longer commands, save a script file in the user's ~/.qwen directory (e.g. ~/.airiscode/statusline-command.sh)
   and use "bash ~/.airiscode/statusline-command.sh" as the command value in settings (no chmod needed).

3. Update the user's ~/.airiscode/settings.json. The statusLine setting is nested under the "ui" key:
   {
     "ui": {
       "statusLine": {
         "type": "command",
