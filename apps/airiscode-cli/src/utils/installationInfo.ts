  ...
  try {
    ...
    if (process.platform === "darwin") {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^airiscode$"', {
          stdio: "ignore",
        });
        return {
    ...
    if (realPath.includes("/.pnpm/global")) {
      const updateCommand = "pnpm add -g @airiscode/cli@latest";
      return {
    ...
    if (realPath.includes("/.yarn/global")) {
      const updateCommand = "yarn global add @airiscode/cli@latest";
      return {
    ...
    if (realPath.includes("/.bun/bin")) {
      const updateCommand = "bun add -g @airiscode/cli@latest";
      return {
    ...
    // Assume global npm
    const updateCommand = "npm install -g @airiscode/cli@latest";
    return {
