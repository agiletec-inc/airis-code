...
export class LspServerManager {
   ...
   * @param handle - The LSP server handle
   */
  private async startServer(name: string, handle: LspServerHandle): Promise<void> {
   ...
   * @param handle - The LSP server handle
   */
  private async doStartServer(name: string, handle: LspServerHandle): Promise<void> {
   ...
   */
  private async stopServer(name: string, handle: LspServerHandle): Promise<void> {
  ...
  private async shutdownConnection(handle: LspServerHandle): Promise<void> {
    ...
    try {
      ...
      if (typeof handle.config.shutdownTimeout === "number") {
        await Promise.race([
          shutdownPromise,
          new Promise<void>((resolve) => setTimeout(resolve, handle.config.shutdownTimeout)),
        ]);
      } else {
  ...
  private buildProcessEnv(env: Record<string, string> | undefined): NodeJS.ProcessEnv | undefined {
  ...
  private async connectSocketWithRetry(
    ...
    timeoutMs: number,
  ): Promise<Awaited<ReturnType<typeof LspConnectionFactory.createSocketConnection>>> {
   ...
   * Create LSP connection
   */
  private async createLspConnection(config: LspServerConfig): Promise<LspConnectionResult> {
    ...
    if (config.transport === "stdio") {
      ...
      return {
        ...
        initialize: async (params: unknown) => lspConnection.connection.initialize(params),
      };
   ...
   * Check path safety
   */
  private isPathSafe(command: string, workspacePath: string, cwd?: string): boolean {
