...
class GrepToolInvocation extends BaseToolInvocation {
    ...
    async performRipgrepSearch(options) {
        ...
        // Add file exclusions from .gitignore and .airiscodeigenore
        const filteringOptions = this.getFileFilteringOptions();
        if (!filteringOptions.respectGitIgnore) {
            rgArgs.push('--no-ignore-vcs');
        }
        if (filteringOptions.respectAiriscodeIgnore) {
            // Load .airiscodeigenore from each workspace directory, not just the primary one
            const seenIgnoreFiles = new Set();
            for (const searchPath of paths) {
                const dir = fs.existsSync(searchPath) && fs.statSync(searchPath).isDirectory()
                    ? searchPath
                    : path.dirname(searchPath);
                const qwenIgnorePath = path.join(dir, '.airiscodeigenore');
    ...
    getFileFilteringOptions() {
        ...
        return {
            ...
            respectAiriscodeIgnore: options?.respectAiriscodeIgnore ??
