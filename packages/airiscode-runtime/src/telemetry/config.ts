...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ?? parseBooleanEnvFlag(env["QWEN_TELEMETRY_ENABLED"]) ?? settings.enabled;
  ...
  const useCollector =
    parseBooleanEnvFlag(env["QWEN_TELEMETRY_USE_COLLECTOR"]) ?? settings.useCollector;
