...
import { act, createContext, createRef, useContext, useEffect, useState } from "react";
...
describe("<VirtualizedList />", () => {
  ...
  describe("with 10px height and 100 items", () => {
    ...
    ])("renders only visible items ($name)", async ({
      initialScrollIndex,
      visible,
      notVisible,
    }) => {
    ...
    ])("mounts only visible items with 1000 items and 10px height (scroll: $initialScrollIndex)", async ({
      initialScrollIndex,
      expectedMountedCount,
    }) => {
      let mountedCount = 0;
