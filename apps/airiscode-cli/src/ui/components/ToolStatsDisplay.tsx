...
export const ToolStatsDisplay: React.FC<ToolStatsDisplayProps> = ({ width }) => {
  ...
  if (activeTools.length === 0) {
    return (
      <Box
        ...
        width={width}
      >
        <Text color={theme.text.primary}>{t("No tool calls have been made in this session.")}</Text>
      </Box>
    );
  }
    ...
    >
      ...
      <Box>
        <Box width={TOOL_NAME_COL_WIDTH + CALLS_COL_WIDTH + SUCCESS_RATE_COL_WIDTH}>
          <Text color={theme.text.link}>{t("Total Reviewed Suggestions:")}</Text>
        </Box>
        <Box width={AVG_DURATION_COL_WIDTH} justifyContent="flex-end">
      ...
      <Box>
        <Box width={TOOL_NAME_COL_WIDTH + CALLS_COL_WIDTH + SUCCESS_RATE_COL_WIDTH}>
          <Text color={theme.text.primary}>{t(" Overall Agreement Rate:")}</Text>
        </Box>
        <Box width={AVG_DURATION_COL_WIDTH} justifyContent="flex-end">
