...
describe('RipGrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should pass .airiscodeigenore to ripgrep when respected', async () => {
            await fs.writeFile(path.join(tempRootDir, '.airiscodeigenore'), 'ignored.txt\n');
            runRipgrep.mockResolvedValue({
                stdout: '',
                truncated: false,
                error: undefined,
        ...
        it('should include .airiscodeigenore matches when disabled in config', async () => {
            await fs.writeFile(path.join(tempRootDir, '.airiscodeigenore'), 'kept.txt\n');
            await fs.writeFile(path.join(tempRootDir, 'kept.txt'), 'keep me');
            Object.assign(mockConfig, {
                getFileFilteringOptions: () => ({
                    respectGitIgnore: true,
                    respectAiriscodeIgnore: false,
                }),
            });
            runRipgrep.mockResolvedValue({
    ...
    describe('multi-directory workspace', () => {
        ...
        it('should load .airiscodeigenore from each workspace directory', async () => {
            const secondDir = await fs.mkdtemp(path.join(os.tmpdir(), 'grep-tool-second-'));
            await fs.writeFile(path.join(secondDir, '.airiscodeigenore'), 'ignored.txt\n');
            await fs.writeFile(path.join(tempRootDir, '.airiscodeigenore'), 'other-ignored.txt\n');
            const multiDirConfig = {
                ...mockConfig,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [secondDir]),
            };
            ...
            const invocation = multiDirGrepTool.build(params);
            await invocation.execute(abortSignal);
            // Verify both .airiscodeigenore files were passed
            const rgArgs = runRipgrep.mock.calls[0][0];
            const ignoreFileArgs = rgArgs.filter((a, i) => i > 0 && rgArgs[i - 1] === '--ignore-file');
            expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.airiscodeigenore'));
            expect(ignoreFileArgs).toContain(path.join(secondDir, '.airiscodeigenore'));
            await fs.rm(secondDir, { recursive: true, force: true });
        });
