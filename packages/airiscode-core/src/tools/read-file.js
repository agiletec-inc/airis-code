...
class ReadFileToolInvocation extends BaseToolInvocation {
    ...
    async execute() {
        const result = await processSingleFileContent(this.params.file_path, this.config, this.params.offset, this.params.limit);
        if (result.error) {
            return {
...
export class ReadFileTool extends BaseDeclarativeTool {
    ...
    validateToolParamValues(params) {
        ...
        if (fileService.shouldAiriscodeIgnoreFile(params.file_path)) {
            return `File path '${filePath}' is ignored by .airiscodeigenore pattern(s).`;
        }
        return null;
    }
