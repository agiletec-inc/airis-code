...
async function main() {
  ...
  try {
    ...
    console.log(
      `   Always-on: filesystem, context7, sequential-thinking, serena, mindbase, self-management`,
    );
    ...
    try {
      ...
      console.log(
        `   ⚠️  Could not enable playwright: ${error instanceof Error ? error.message : String(error)}`,
      );
