...
import { CodingPlanRegion, isCodingPlanConfig } from "../../constants/codingPlan.js";
...
const MODEL_PROVIDERS_DOCUMENTATION_URL =
  "https://qwenlm.github.io/airiscode-docs/en/users/configuration/model-providers/";

function parseDefaultAuthType(defaultAuthType: string | undefined): AuthType | null {
...
const ALIBABA_STANDARD_API_DOCUMENTATION_URLS: Record<AlibabaStandardRegion, string> = {
...
export function AuthDialog(): React.JSX.Element {
  ...
  const [region, setRegion] = useState<CodingPlanRegion>(CodingPlanRegion.CHINA);
  const [alibabaStandardRegionIndex, setAlibabaStandardRegionIndex] = useState<number>(0);
  ...
  const [alibabaStandardApiKeyError, setAlibabaStandardApiKeyError] = useState<string | null>(null);
  const [alibabaStandardModelId, setAlibabaStandardModelId] = useState("");
  const [alibabaStandardModelIdError, setAlibabaStandardModelIdError] = useState<string | null>(
    null,
  );

  // Main authentication entries (flat layout)
  const mainItems = [
    {
      key: "CODING_PLAN",
  ...
  const regionItems = [
    {
      ...
      description: (
        <Link url="https://help.aliyun.com/zh/model-studio/coding-plan" fallback={false}>
    ...
    },
    {
      ...
      description: (
        <Link url="https://www.alibabacloud.com/help/en/model-studio/coding-plan" fallback={false}>
          <Text color={theme.text.secondary}>
  ...
  const isCurrentlyCodingPlan =
    isCodingPlanConfig(contentGenConfig?.baseUrl, contentGenConfig?.apiKeyEnvKey) !== false;
  ...
  const handleAlibabaStandardRegionSelect = async (selectedRegion: AlibabaStandardRegion) => {
    setErrorMessage(null);
    onAuthError(null);
    setAlibabaStandardApiKeyError(null);
    setAlibabaStandardModelIdError(null);
  ...
  const handleAlibabaStandardModelSubmit = () => {
    ...
    setAlibabaStandardModelIdError(null);
    void handleAlibabaStandardSubmit(trimmedApiKey, alibabaStandardRegion, trimmedModelIds);
  };
    ...
    (key) => {
      if (key.name === "escape") {
        ...
        if (config.getAuthType() === undefined) {
          setErrorMessage(
            t("You must select an auth method to proceed. Press Ctrl+C again to exit."),
          );
          return;
        }
  ...
  const renderApiKeyInputView = () => (
    <Box marginTop={1}>
      <ApiKeyInput onSubmit={handleApiKeyInputSubmit} onCancel={handleGoBack} region={region} />
    </Box>
  );
    ...
    <>
      <Box marginTop={1}>
        <DescriptiveRadioButtonSelect
          ...
          onHighlight={(value) => {
            const index = apiKeyTypeItems.findIndex((item) => item.value === value);
    ...
    <>
      <Box marginTop={1}>
        <DescriptiveRadioButtonSelect
          ...
          onHighlight={(value) => {
            const index = alibabaStandardRegionItems.findIndex((item) => item.value === value);
  ...
  const renderAlibabaStandardApiKeyInputView = () => (
    <Box marginTop={1} flexDirection="column">
      ...
      <Box marginTop={0}>
        <Link url={ALIBABA_STANDARD_API_DOCUMENTATION_URLS[alibabaStandardRegion]} fallback={false}>
    ...
    <>
      ...
      <Box marginTop={0}>
        <Link url={MODEL_PROVIDERS_DOCUMENTATION_URL} fallback={false}>
          <Text color={theme.text.link}>{MODEL_PROVIDERS_DOCUMENTATION_URL}</Text>
        </Link>
      </Box>
      <Box marginTop={1}>
  ...
  const getViewTitle = () => {
    switch (viewLevel) {
      ...
      case "alibaba-standard-region-select":
        return t("Select Region for Alibaba Cloud ModelStudio Standard API Key");
    ...
    >
      ...
      {viewLevel === "main" && (
        <>
          ...
          <Box>
            <Text color={theme.text.primary}>{t("Terms of Services and Privacy Notice")}:</Text>
          </Box>
          <Box>
            <Link
              url="https://qwenlm.github.io/airiscode-docs/en/users/support/tos-privacy/"
              fallback={false}
            >
              <Text color={theme.text.secondary} underline>
                https://qwenlm.github.io/airiscode-docs/en/users/support/tos-privacy/
              </Text>
            </Link>
          </Box>
        </>
