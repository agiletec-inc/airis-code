...
export class GitWorktreeService {
   ...
   */
  async setupWorktrees(config: WorktreeSetupConfig): Promise<WorktreeSetupResult> {
   ...
   */
  async removeWorktree(worktreePath: string): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  async getWorktreeDiff(worktreePath: string, baseBranch?: string): Promise<string> {
    const worktreeGit = simpleGit(worktreePath);

    const base =
      (await this.resolveBaseline(worktreeGit)) ?? baseBranch ?? (await this.getCurrentBranch());
   ...
   */
  private async resolveBaseline(worktreeGit: SimpleGit): Promise<string | null> {
    try {
      const sha = (
        await worktreeGit.raw(["log", "--grep", BASELINE_COMMIT_MESSAGE, "--format=%H", "-1"])
      ).trim();
      return sha || null;
    } catch {
      return null;
  ...
  private async withStagedChanges<T>(git: SimpleGit, fn: () => Promise<T>): Promise<T> {
    await git.add(["--all"]);
    try {
      return await fn();
    } finally {
