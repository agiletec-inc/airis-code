 ...
 */
export async function resolveRipgrep(useBuiltin: boolean = true): Promise<RipgrepSelection | null> {
 ...
 */
export async function ensureRipgrepHealthy(selection: RipgrepSelection): Promise<void> {
...
export async function ensureMacBinarySigned(selection: RipgrepSelection): Promise<void> {
 ...
 * @throws {Error} If an error occurs while resolving the ripgrep binary.
 */
export async function canUseRipgrep(useBuiltin: boolean = true): Promise<boolean> {
 ...
 */
export async function runRipgrep(args: string[], signal?: AbortSignal): Promise<RipgrepRunResult> {
  ...
  return new Promise<RipgrepRunResult>((resolve) => {
    const child = execFile(
      ...
      (error, stdout = "", stderr = "") => {
        ...
        const errorCode = (error as Error & { code?: string | number | undefined | null }).code;
        if (errorCode === 1) {
