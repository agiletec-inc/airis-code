...
vi.mock('os');
const MEMORY_SECTION_HEADER = '## Auto-saved Memories';
describe('MemoryTool', () => {
    ...
    describe('performAddMemoryEntry (static method)', () => {
        let testFilePath;
        beforeEach(() => {
            testFilePath = path.join(os.homedir(), '.airiscode', DEFAULT_CONTEXT_FILENAME);
        });
    ...
    describe('execute (instance method)', () => {
        ...
        it('should call performAddMemoryEntry with correct parameters and return success for global scope', async () => {
            ...
            const expectedFilePath = path.join(os.homedir(), '.airiscode', getCurrentGeminiMdFilename());
    ...
    describe('getDefaultPermission and getConfirmationDetails', () => {
        ...
        it('should return confirmation details for global scope', async () => {
            ...
            if (result.type === 'edit') {
                const expectedPath = path.join('~', '.airiscode', 'AIRISCODE.md');
                expect(result.title).toBe(`Confirm Memory Save: ${expectedPath} (global)`);
                expect(result.fileName).toContain(path.join('mock', 'home', '.airiscode'));
                expect(result.fileName).toContain('AIRISCODE.md');
                expect(result.fileDiff).toContain('Index: AIRISCODE.md');
                expect(result.fileDiff).toContain('+## Auto-saved Memories');
                expect(result.fileDiff).toContain('+- Test fact');
                expect(result.originalContent).toBe('');
                expect(result.newContent).toContain('## Auto-saved Memories');
        ...
        it('should return confirmation details for project scope', async () => {
            ...
            expect(result).toBeDefined();
            if (result.type === 'edit') {
                const expectedPath = path.join(process.cwd(), 'AIRISCODE.md');
        ...
        it('should have no-op onConfirm callback', async () => {
            ...
            if (result.type === 'edit') {
                ...
                await expect(result.onConfirm(ToolConfirmationOutcome.ProceedAlways)).resolves.toBeUndefined();
        ...
        it('should prompt for scope selection when scope is not specified', async () => {
            ...
            if (result.type === 'edit') {
                ...
                expect(result.fileName).toBe('AIRISCODE.md');
                expect(result.fileDiff).toContain('Test fact');
                expect(result.fileDiff).toContain('--- AIRISCODE.md');
                expect(result.fileDiff).toContain('+++ AIRISCODE.md');
                expect(result.fileDiff).toContain('+- Test fact');
        ...
        it('should show correct file paths in scope selection prompt', async () => {
            ...
            expect(result).toBeDefined();
            if (result.type === 'edit') {
                const globalPath = path.join('~', '.airiscode', 'AIRISCODE.md');
                const projectPath = path.join(process.cwd(), 'AIRISCODE.md');
