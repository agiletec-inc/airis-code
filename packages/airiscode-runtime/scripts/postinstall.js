...
function setupRipgrepBinaries() {
  try {
    ...
    if (!binaryDir || !fs.existsSync(binaryDir)) {
      console.log(`ℹ Binary directory not found for ${platform}-${arch}, skipping ripgrep setup`);
      return;
    }
    ...
    try {
      ...
      console.log(`⚠ Could not complete ripgrep setup: ${error.message || "Unknown error"}`);
      console.log("  This is not critical - ripgrep may still work correctly");
    }
  } catch (error) {
    console.log(`⚠ Ripgrep setup encountered an issue: ${error.message || "Unknown error"}`);
