...
export async function loadCliConfig(
  ...
  // Pass cwd so that relative paths like ".airiscode" resolve per-project.
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`airiscode --resume\` without an ID to choose from existing sessions.`;
