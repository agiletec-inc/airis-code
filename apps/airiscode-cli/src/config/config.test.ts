...
import * as ServerConfig from '@airiscode/core';
...
describe('loadCliConfig', () => {
  ...
  it('should reset context file names to AIRISCODE.md and AGENTS.md by default', async () => {
  ...
  it('should reset context filenames to defaults when context.fileName is not configured', async () => {
    ...
    const argv = await parseArguments();
    const settings: Settings = {};
    const defaultContextFiles = ['AIRISCODE.md', 'AGENTS.md'];
    const getAllSpy = vi
...
describe('loadCliConfig model selection', () => {
  ...
  it.skip('uses the default gemini model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_AIRISCODE_MODEL);
  });
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectAiriscodeIgnore',
...
describe('loadCliConfig runtimeOutputDir', () => {
  ...
  it('should resolve relative runtimeOutputDir against cwd', async () => {
    const argv = await parseArguments();
    const settings: Settings = {
      advanced: { runtimeOutputDir: '.airiscode' },
    };
