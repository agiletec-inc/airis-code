...
export class StandardFileSystemService implements FileSystemService {
  ...
  async writeTextFile(
    ...
    const content = shouldUseCrlf ? ensureCrlfLineEndings(params.content) : params.content;
    ...
    const isNonUtf8Encoding =
      encoding && !isUtf8CompatibleEncoding(encoding) && iconvEncodingExists(encoding);
    ...
    if (isNonUtf8Encoding) {
      ...
      if (bom) {
        ...
        await fs.writeFile(filePath, bomBytes ? Buffer.concat([bomBytes, encoded]) : encoded);
      } else {
