...
function record(method: string, language: string, passed: boolean, detail: string): void {
...
async function testLanguage(cfg: LanguageTestConfig): Promise<void> {
  ...
  const isServerLimited = (method: string) => serverLimitedMethods?.has(method) ?? false;
      ...
      if (callItems.length > 0) {
        ...
        try {
          ...
          record(
            ...
            langName,
            incoming.length > 0,
            incoming.length > 0 ? `${incoming.length} caller(s)` : "Empty (no callers found)",
          );
        } catch (e: any) {
...
async function main(): Promise<void> {
  ...
  for (const m of methods) {
    ...
    const langs = mr.map((r) => (r.passed ? green(r.language) : red(r.language))).join(" | ");
    console.log(`  ${m}: ${langs}`);
  }
