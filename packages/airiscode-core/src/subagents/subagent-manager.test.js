...
describe('SubagentManager', () => {
    ...
    const validConfig = {
        ...
        level: 'project',
        filePath: '/test/project/.airiscode/agents/test-agent.md',
    };
    const validMarkdown = `---
name: test-agent
description: A test subagent
...
`;
        ...
        it('should determine level from file path', () => {
            const projectPath = '/test/project/.airiscode/agents/test-agent.md';
            const userPath = '/home/user/.airiscode/agents/test-agent.md';
...
`;
        ...
        it('should not warn when filename matches subagent name', () => {
            ...
            const matchingPath = '/test/project/.airiscode/agents/test-agent.md';
    ...
    describe('loadSubagent', () => {
        it('should load subagent from project level first', async () => {
            ...
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents/test-agent.md'), 'utf8');
        });
        it('should fall back to user level if project level fails', async () => {
            ...
            expect(config).toBeDefined();
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/home/user/.airiscode/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/home/user/.airiscode/agents/test-agent.md'), 'utf8');
        });
...
You are another assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.airiscode/agents/wrong-filename.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/home/user/.airiscode/agents/user-agent.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.airiscode/agents/misnamed-file.md'));
        });
    });
    ...
    describe('deleteSubagent', () => {
        it('should delete subagent from specified level', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents/test-agent.md'));
        });
        it('should delete from both levels if no level specified', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents/test-agent.md'));
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/home/user/.airiscode/agents/test-agent.md'));
        });
        it('should throw error if subagent not found', async () => {
            vi.mocked(fs.readdir).mockRejectedValue(new Error('Directory not found'));
...
You are a helpful assistant.`;
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents/wrong-name.md'));
        });
...
Target content`,
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.airiscode/agents/target-file.md'));
        });
    });
    ...
    describe('Runtime Configuration Methods', () => {
        describe('convertToRuntimeConfig', () => {
            ...
            it('should accept cross-provider model selectors', () => {
                ...
                const runtimeConfig = manager.convertToRuntimeConfig(configWithCrossProvider);
