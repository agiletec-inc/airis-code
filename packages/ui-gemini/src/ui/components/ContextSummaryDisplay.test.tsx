...
describe("<ContextSummaryDisplay />", () => {
  ...
  it("should switch layout at the 80-column breakpoint", () => {
    ...
    const { lastFrame: wideFrame, unmount: unmountWide } = renderWithWidth(80, baseProps);
    expect(wideFrame()!.includes("\n")).toBe(false);
    unmountWide();

    // At 79 columns, should be on multiple lines
    const { lastFrame: narrowFrame, unmount: unmountNarrow } = renderWithWidth(79, baseProps);
