...
describe('ReadFileTool', () => {
    ...
    beforeEach(async () => {
        ...
        const mockConfigInstance = {
            ...
            storage: {
                ...
                getUserSkillsDirs: () => [path.join(os.homedir(), '.airiscode', 'skills')],
            },
    ...
    describe('execute', () => {
        ...
        describe('with .airiscodeigenore', () => {
            beforeEach(async () => {
                await fsp.writeFile(path.join(tempRootDir, '.airiscodeigenore'), ['foo.*', 'ignored/'].join('\n'));
            });
            it('should throw error if path is ignored by a .airiscodeigenore pattern', async () => {
                const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
                await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
                const params = {
                    file_path: ignoredFilePath,
                };
                const expectedError = `File path '${ignoredFilePath}' is ignored by .airiscodeigenore pattern(s).`;
